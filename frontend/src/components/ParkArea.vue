...
<v-card style="width:300px; margin-left:5%;" outlined>
    ...
    <v-card-title v-if="value._links">
        ParkArea # {{value._links.self.href.split("/")[value._links.self.href.split("/").length - 1]}}
    </v-card-title >
    <v-card-title v-else>
        ParkArea
    </v-card-title >
    ...
    <v-card-text style = "margin-left:-15px; margin-top:10px;">
          ...
          <div class="grey--text ml-4" v-if="editMode" style = "margin-top:-20px;">
            <v-text-field label="ParkingZoneNo" v-model="value.parkingZoneNo"/>
          </div>
          <div class="grey--text ml-4" v-else>
            ParkingZoneNo :  {{value.parkingZoneNo }}
          </div>


          <div class="grey--text ml-4" v-if="editMode" style = "margin-top:-20px;">
            <v-text-field type="number" label="ParkingId" v-model="value.parkingId"/>
          </div>
          <div class="grey--text ml-4" v-else>
    ...
    methods: {
      ...
      async save(){
        try{
          var temp = null;
          if(this.isNew){
            temp = await axios.post(axios.fixUrl('/parkAreas'), this.value)
          }else{
